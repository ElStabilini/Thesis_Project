...
\vspace{0.75em}
  ...
  \begin{tikzpicture}[
    ...
    {\draw[arrow] (logical) -- node[label] {Transpilation} (native);}
    {\draw[arrow] (native) -- node[label] {Compilation} (pulses);}
...
\begin{frame}[t,fragile]{RB optimization \hfill{\small[arXiv:1403.0035]}}
  ...
  \begin{tikzpicture}[
    ...
    {\draw[arrow] (logical) -- node[label] {Fine-tuning} (native);}
    {\draw[arrow] (native) -- node[label] {Optimization} (pulses);}
...
\begin{frame}{Improving Qubit Control for Calibration}
  ...
  \begin{itemize}
    ...
    \item We can try to reduce residual erros by adding calibrating $R_X(\pi/2)$ as native gate. 
  \end{itemize}
...
\begin{frame}{Native RX90}
  \begin{itemize}
    ...
    \item Problem: there may be nonlinearities and distortions in signals that degradete the $R_X(\frac{\pi}{2})$ fidelity.
  \end{itemize}
  \begin{figure}
    \centering
