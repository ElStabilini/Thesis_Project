...
For each of the calibration experiments concerning the rotation pulses, I provide plots illustrating how the relevant physical quantities vary depending on whether I am calibrating the $R_X(\pi)$ or the $R_X(\pi/2)$ gate.
In addition to the graphical data, a summary table is provided reporting the measured amplitude (or duration) of the rotation pulse along with the associated measurement uncertainty derived from the calibration procedure.
In the table I included also the standardized difference metric, defined as:
\begin{equation}\label{eq:std_difference}
    Z = \frac{v_{RX/2}-v_{RX90}}{\sqrt{\sigma^2_{RX90} + \sigma^2_{RX/2}}}
\end{equation}
where $v_{RX/2}$ is the physical quantity measured by halving the value corresponding to the $R_X(\pi)$ pulse, $v_{RX90}$ is the physical quantity obtained from a direct calibration of the $R_X(\pi/2)$ gate, $\sigma^2_{RX/2}$ and $\sigma^2_{RX90}$ are the respective measurement variances.
...
Table \ref{tab:amplitude} reports the results from Rabi experiments performed to calibrate the amplitude of the pulses associated with the $R_X(\pi)$ and $R_X(\pi/2)$ gates.
...
Due to motivi pratici e di tempo the pulse duration measurements for the $R_X(\pi)$ and $R_X(\pi/2)$ gates were carried out on the Soprano-D chip by QuantWare \cite{qw5q}. From here on, this device will be referred to as qw5q when necessary.
As with the amplitude measurements, Table \ref{tab:amplitude} summarizes the results from Rabi experiments conducted to calibrate the duration of the pulses associated with the $R_X(\pi)$ and $R_X(\pi/2)$ gates.\\
Again, both the inferred value (obtained by halving the $R_X(\pi)$ duration) and the directly calibrated value for the $R_X(\pi/2)$ gate are shown. The sequence of measurements mirrored that used for the amplitude calibration: 
$R_X(\pi)$ was calibrated first, followed by $R_X(\pi/2)$, with all experimental settings kept fixed except for the \tt{rx90} flag.
...
The only exception to this trend occurs for qubit \texttt{5}, for which the standardized difference is negative.
...
This reduction in $|Z|$ can be attributed to two factors: the smaller absolute difference between the duration values obtained via direct and indirect calibration, and the larger relative uncertainties associated with the duration estimates compared to those for amplitude.
...
In Figure \ref{fig:af_comparison} the results of the Rabi amplitude-frequency calibration experiments, which aim to determine the correct amplitude and frequency for either the $R_X(\pi)$ or $R_X(\pi/2)$ gate, with a fixed pulse duration of 40 ns.
...
However, when extending the scan ranges or, as in our case, using the same scan intervals for a gate that ideally requires roughly half the amplitude and frequency, the resulting pattern deviates from the usual single-peak expectation. 
Instead, we observe a more complex structure, consistent with the model described by Equation \ref{eq:P1_rabi}, in which both amplitude and frequency are varied simultaneously.
...
However, with such an interference pattern, the standard peak detection algorithm fails to correctly identify the first peak, as evidenced by the misalignment of the dashed line in Figure \ref{fig:B2_90}. 

To enable a quantitative comparison between the $R_X(\pi/2)$ amplitude obtained through direct calibration and the value inferred from the $R_X(\pi)$ calibration, additional measurements were carried out using modified scan ranges for amplitude and frequency.
...
For the calibration of the $R_X(\pi)$ gate, the frequency was scanned over a [??] MHz range centered around the qubit frequency, with a step size of 1 MHz. 
The amplitude was scanned in the interval [??] using a resolution of [??].
The results of these measurements are summarized in Table \ref{tab:af}, and an example of the corresponding interference patterns is shown in Figure \ref{fig:qw5q}.
...
It is important to emphasize, however, that this apparent agreement is largely a consequence of the relatively large measurement uncertainty associated with the amplitude values obtained via the \tt{rabi\_amplitude\_frequency} routine.
The typical relative error in these estimates is on the order of [??]
...
\newpage
\newgeometry{a4paper, top=2.5cm, bottom=2.5cm, inner=2.5cm, outer=1.5cm, heightrounded}
...
In Figure \ref{fig:RXvsRX90} I show three plots, each comparing the pulse parameters obtained for implementing an $R_X(\pi/2)$ gate using different calibration strategies, across the various qubits for which the \tt{rx90} calibration was performed.

Un'osservazione interessante, già menzionata prima ma che diventa evidente guardando i plots is that both the amplitude and duration estimates for the $R_X(\pi/2)$ gate, when obtained by halving the corresponding values calibrated for the $R_X(\pi)$ gate, tend to be slightly overestimated compared to those obtained via direct calibration of the $R_X(\pi/2)$ gate.
...
Nevertheless, it is important to emphasize that these last amplitude values are also associated with larger relative uncertainties. 
...
The first observation from these plots is that the IIR filter effectively corrects the exponential rise in the step response. %TODO: aggiungere miglioramento quantitativo
...
%TOD0: quantificare la correzione e commentare risultato.
...
The first observation, consistent with the results shown in Figures \ref{fig:inverse_short}, \ref{fig:inverse:long}, and \ref{fig:IIR}, is that there is likely an issue with signal normalization. 
...
A second important consideration concerns the optimization strategy using the \tt{CMA-ES} algorithm. 
