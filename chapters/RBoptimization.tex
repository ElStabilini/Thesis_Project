...
Some optimization runs were performed with this revised configuration.
...
Questo stesso comportamente sarà evidente anche negli altri plot che mostrano il progresso dell'ottimizzazione per questo algoritmo (see Figure \ref{fig:20241113_181711:fidelity}). 
...
In particolare, per quanto riguarda l'ottimizzazione indicata con \tt{init\_symp\_2} the most interesting aspect is again il fatto che a parametri quasi fissati, quindi qausi a parità di ampiezza e frequenza quello che si osserva sono valori di fidelity molto distanti.
Si considerino in particolare gli step di ottimizzazione per cui si osserva un picco negativo della fidelity in Figure \ref{fig:20241113_181711:fidelity}, looking at these same optimization steps in Figure \ref{fig:20241113_181711:parameters} è evidente che ampiezza e frequenza non variate notevolmente.
Ancora una volta un valore di fidelity così basso è da attribuire ad un fallimento della routine di Randomized Benchmarking.
...
In questo caso, soprattutto se si considera (la frequenza si stabilizza al valore minimo, è possibile che sia fallito Ramsey nonostante i range e tutto fossero stati calibrati in precedenza)
