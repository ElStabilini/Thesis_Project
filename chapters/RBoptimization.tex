...
This anomaly is due to a failure in the fitting procedure used to estimate the randomized benchmarking infidelity at that step (see Figure [TODO: insert reference]).
A similar behavior can be seen in other plots depicting the optimization progress for this algorithm (see Figure \ref{fig:20241113_181711:fidelity}), indicating that such fitting failures can occasionally impact the apparent fidelity trend during the optimization process.
...
Once again, such low fidelity values are attributed to failures in the Randomized Benchmarking fitting procedure, rather than real changes in gate performance.
...
However, this assumption was disproven by the diagnostic report automatically generated for each operation performed with \Qibocal.
The corresponding Ramsey experiment, which confirms successful execution, is shown in \hyperref[app:AppendixC]{Appendix C}, Figure \ref{fig:Ramsey_NM}.
...
In generale è evidente che l'ottimizzazione anche del parametro $\beta$ per il DRAG ha consentito il raggiungimento di avarage Clifford gate fidelity più alte, per questo motivo nei tentativi di ottimizzazione successivi si è deciso di re-introdurre l'ottimizzazione del parametro $\beta$.
...
\begin{table}[h]
    ...
    \begin{tabular}{lcccccc}
        ...
        \midrule
        \textbf{CMA-ES} & 0.002732 & 30 & 210 & 4 536 \\
        \textbf{Optuna 30 steps} &  0.004593 & 30 & 30 & 763\\
        \textbf{Optuna 100 steps} & 0.004297 & 100 & 100 & 2 257\\
        \textbf{Optuna 1000 steps} & 0.004507 & 1000 & 1000 & 21 811\\
        \textbf{Optuna no $\beta$} & 0.004861 & 1000 & 1000 & 21 820\\
        \bottomrule
    \end{tabular}
