...
In particular, regarding the optimization labeled as \tt{init\_symp\_2}, the most important observation is again the presence of large variations in fidelity despite nearly fixed parameters. 
...
In this third and final case, the lack of convergence is apparent directly from the fidelity plot shown in Figure \ref{fig:20241113_200745:fidelity}.
...
%TODO: completare studio del confronto
...
Il confronto tra i valori ottenuti dall'ottimizzazione con i diversi metodi sono riportati in Figure [TODO:insert ref]. 
Dai risultati è evidente che i parametri sono soggetti a drift e che probabilmente la calibrazione ottenuta non è particolarmente stabile.
...
As a first attempt at applying the optimization algorithm implemented in the \texttt{Optuna} library, a total of 30 optimization steps, or trials as referred in the library itself, were performed. 
...
Unlike gradient-based methods or some evolutionary strategies, Optuna is more exploratory by design and may not settle into a stable optimum within a fixed number of trials.

Instead, Optuna proves valuable as a tool for systematically exploring the parameter space. 
...
In particular, the best average Clifford gate fidelity of 99.64\% was obtained at the 13th trial.
...
An analysis of the plots and the numerical values obtained for the $R_X(\pi)$ pulse parameters shows that the highest Clifford gate avarage fidelity corresponds to distinct values of these parameters. 
%TODO: inserire valori per ampiezza e frequenza per i due esperimenti precedenti
This effect is particularly evident for the amplitude and $\beta$ parameters. 
While the amplitude range remains fixed, being constrained by hardware limitations, the range used to search for the optimal $\beta$ varies significantly across different optimization runs.
...
its proper calibration does have a measurable impact on the shape of the control pulse and, therefore, on the overall gates performance.

The reason for the significant variation in the allowed search ranges for $\beta$ can be traced back to a failure in the fine-tuning routine responsible for calibrating the DRAG parameter.
...
In consideration of the relatively short time required by Optuna in the previous attempt, approximately 40 minutes for 100 trials, we decided to significantly increase the number of optimization trials. 
Specifically, we set an upper limit of 1000 trials, with the goal of improving parameter space exploration and potentially achieving better convergence.

The optimization history is shown in Figure \ref{fig:optuna1000beta}. 
At first the results appear promising: the algorithm seems to achieve an average Clifford gate fidelity of 99.98\%. 
However a closer examination of the outputs corresponding to all RB routines with reported avarage Clifford gate infidelities below $1.5 \cdot 10^{-3}$, it becomes evident that these results are actually the consequence of failed exponential fits on the RB output.
...
This outcome corresponds to an improvement of approximately [TODO: insert percentage improvement]\% compared to the initial configuration.
...
Per quanto riguarda invece ampiezza e frequenza, il fatto che i valori migliori si trovino nella zona più densamente campionata ci tranquillizza circa l'effettivo funzionamento dell'ottimizzazione con optuna.
...
Overall, i risultati mostrati fino a qui suggests that the optimization process carried out by \tt{Optuna} does not exhibit clear convergence.
...
The results obtained from various optimization strategies—Nelder-Mead, CMA-ES, and Optuna—demonstrate that while meaningful improvements in fidelity are achievable, the current implementation does not yet meet the robustness and efficiency requirements for routine deployment. 
The primary bottleneck remains the cost of the Randomized Benchmarking (RB) routine, which is necessary for fidelity evaluation. Because each optimization requires multiple evaluations of this cost function, the total execution time quickly becomes a limiting factor.

Additionally, we found that convergence is not guaranteed. 
In several cases, especially those involving Optuna or CMA-ES with a broader parameter search, the optimization did not yield stable or reproducible results.
...
Two directions for future improvement emerged from this work. 
...
For example, in the case of CMA-ES, adjusting the population size or reducing the initial value of $\sigma$ could improve convergence, albeit at the cost of reduced parameter space exploration or increased computational load. 
...
\end{comment}
%TODO: completare tabelle per optuna
%TODO: correggere tabelle per optuna (cambiare la miglior fidelity raggiunta se quella raggiunta non è davvero la miglior fidelity)
...
\begin{sidewaystable}[h]
    ...
    \begin{tabular}{lccccccccccc}
        ...
        \textbf{optuna no $\beta$} & 0.006983 & 0.00153 & 870 & 0.04104 & 4.957374 & - & 77.96 & 0.99301 & 0.99846 & 0.54\\
        \bottomrule
    \end{tabular}
    \caption{The table contain the results ottenuti dall'ottimizzazione dei}
...
\begin{sidewaystable}[h]
    ...
    \begin{tabular}{lcccccccccc}
        ...
        \textbf{optuna\_1000} & 0.005307 & 0.003424 & -0.042718 & 4.95794 & 0.0626649 & 35.48 & 0.99469 & 0.99657 & 0.1893\\
        \textbf{optuna no $\beta$} & 0.006983 & 0.137907 & 0.058214 & 4.95818 & - & -1874.74 & 0.99301 & 0.86209 & -13.184\\
        \bottomrule
    \end{tabular}
    \caption{}
