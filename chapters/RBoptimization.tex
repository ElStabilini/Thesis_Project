...
Accurate control of single-qubit gates is a fundamental requirement for quantum computing. 
The single-gate fidelity quantifies the average performance of individual gates and directly impacts the error rates of quantum circuits and is a critical figure of merit.

The calibration procedure detailed in Section \ref{sec:calibration} typically requires significant time and experimental expertise, especially to obtain state-of-the art calibration.
In my work, the calibration started from a pre-existing, though non optimal, configuration and the task consisted of improving the quality the gates by using available \Qibocal protocols.
Even under these conditions, the effort required to refine gate performance highlighted the limitations of manual methods.

As quantum processors scale in size and complexity, manual calibration becomes increasingly impractical. 
Frequent recalibrations are necessary to mitigate the effects of parameter drift and environmental fluctuations, especially in superconducting devices \cite{krantz_quantum_2019}.

In this chapter, I present the results of an optimization procedure aimed at enhancing the fidelity of single-qubit gates on a superconducting quantum device.
The focus was placed on the optimization of single-qubit gate fidelity, as it represents both a fundamental and relatively accessible figure of merit. 
High single-qubit gate fidelities are essential for the reliable construction of quantum circuits, and their characterization and improvement are comparatively simpler than for two-qubit gates. 
Moreover, considering the final goal of developing automated calibration procedures, a natural first step is to automate the re-calibration and optimization of existing gate parameters, building on a pre-existing calibration. 
...
The survival probability of the initial state $\ket{0}$ for different sequence lengths $m$ follows the exponential decay model \begin{equation}
    F(m) = Ap^m +B,
\end{equation}
...
Now we can derive the avarage error per Clifford gate $\varepsilon_{Clifford}$ \begin{equation}
...
For the results we present in the following the technique used slightly differs from the one described in section \ref{sec:RBsection} 
